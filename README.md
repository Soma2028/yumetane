...
- **推薦の中心はカード選択アルゴリズム**: 右スワイプした職業の重心にユーザーの
  位置を推定し、残りの候補を(1)推定位置との類似度(exploit)、(2)これまでに
  見せたカード全件との最小距離(explore)、(3)認知度「知らない」職業への
  優先度(awareness_bonus)の3項でスコアリングして次の1枚を選ぶ。序盤は探索寄りに
  し、終盤でも類似度重視の重みに上限を設けることで、常に一定割合の探索と
  未知の職業が混ざるようにしている（重みの値はいずれも暫定値で未検証。詳細は
  [docs/design.md](docs/design.md)）
- 上記の土台として、job tag（日本版O-NET、独立行政法人労働政策研究・研修機構が
  公開する職業情報データベース）を加工し、職業興味（RIASEC）・仕事内容などの
  数値データから推薦用の特徴量を構築。RIASECデータを持つ518職業中36件で欠損が
  あり、知識・仕事の性質からRidge回帰で補完しようとしたところ、入力が全欠損の
  7件が同一の予測値になるバグを検証で発見し、`riasec_source`（実測／推定／
  推定不能）の3値管理で対処した
- 「子どもの職業認知度」を測った公開データは存在しないため、Wikipediaの
  記事有無・ページビューを代理指標として検証したが機能せず、最終的に
  1名による自作ラベル（層化抽出した179職業）に切り替えた
- 当初はRIASEC 6因子の相関構造をPCAで分析し、直交する主成分ベースで10問の
  質問に答える形式を設計・実装・自己検証まで完了させたが、既存の適職診断
  アプリと体験として見分けがつかず「知らない職業に出会う」という目的が体験に
  現れていないと判断し、現在のスワイプ形式に置き換えた（設計と検証の記録は
  `docs/questions.md`に保持）
